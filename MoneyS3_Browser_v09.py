...
APP_TITLE = "Money S3 Browser v0.9"
DEFAULT_ROOTS = [
...
class MoneyBrowserApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        top.pack(fill="x")
        ttk.Label(top, text="Money S3 Browser v0.9", style="InvoiceTitle.TLabel").pack(
            side="left", padx=(0, 12)
        )
    ...
    def _party_from_prefix(self, row, prefix):
        return {
            "name": first_by_alias(row, [f"{prefix}_Nazev", f"{prefix}_NazevOb", f"{prefix}_Jmeno"]),
            "street": first_by_alias(row, [f"{prefix}_Ulice", f"{prefix}_UliceOb"]),
            "city": first_by_alias(row, [f"{prefix}_Mesto", f"{prefix}_MestoOb"]),
            "psc": first_by_alias(row, [f"{prefix}_Psc", f"{prefix}_PscOb"]),
            "state": first_by_alias(row, [f"{prefix}_Stat", f"{prefix}_StatOb"]),
            "ico": first_by_alias(row, [f"{prefix}_ICO"]),
            "dic": first_by_alias(row, [f"{prefix}_DIC"]),
        }
